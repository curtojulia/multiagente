# Documentação do Sistema: Verificador de Código IA
...
1. **Gerar README.md**: Cria uma documentação do códigocom base no que foi colado pelo usuário.
2. **Analisar Segurança**: Verifica o código em busca de vulnerabilidades de segurança.
3. **Otimizar Performance**: Analisa o código e sugere melhorias para otimização de performance.
...
1. **Clone o repositório** ou gere um novo arquivo Python (`verificador_codigo.py`) e cole o código fornecido nele.

2. **Instale as dependências** conforme as instruções acima.

3. **Crie um arquivo `.env`** na raiz do diretório do seu projeto, e adicione suas chaves de API necessárias, caso sejam requeridas pelos agentes.

4. **Execute a aplicação Streamlit** digitando o seguinte comando no terminal:
...
5. **Abra o navegador**: Após executar o comando, siga o link que aparecerá no terminal (normalmente http://localhost:8501) para acessar a aplicação.

6. **Utilize a aplicação**: Cole seu código na área designada, selecione uma opção no menu lateral e clique no botão "Executar" para ver os resultados.
